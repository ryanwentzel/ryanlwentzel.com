---
title: Get the Latest Content
description: "Subscribe to ryanlwentzel.com to get the latest content."
---
Sign up to receive the latest content from ryanlwentzel.com in your inbox.

<div class="form">
  <form action="https://buttondown.email/api/emails/embed-subscribe/ryanlwentzel.com" method="post" class="embeddable-buttondown-form">
    <div class="form__item">
      <label class="form__label" for="bd-email">Email address</label>
      <input class="form__input" type="email" name="email" id="bd-email" placeholder="Your email (you@example.com)" />
    </div>
      <input type="hidden" value="1" name="embed" />
    <div class="form__item">
      <input class="button button--large" type="submit" value="Subscribe" />
    </div>
  </form>
</div>

Prefer to use a feed reader? Subscribe to the [Atom](/feeds/feed.xml) feed ([what's a feed?](https://aboutfeeds.com/)).

